<script>
 ...
 <!--  THE MODAL -->
 <div style='margin-top:5%;' class='modal fade' id='modal-container-admin' role='dialog' aria-labelledby='myModalLabel' aria-hidden='false'>
 ...
 <div id='template_view_1' class='container animated fadeIn kioview' style='text-align:center;'  >
 	<div class='xxxpagination_page' >
 		<div class='row top-row'>
 			<div class='col-md-12'>
 				<h3>admin   <?php echo($_SESSION['admin_id']); ?></h3>

  <button class='create_btn' >+</button>
				<!-- hidden anchor that calls the modal -->
 				<a id='modal-admin' style='' href='#modal-container-admin' role='button' class='btn invisible' data-toggle='modal'>modal-1</a>
 				<!-- ======================================================  -->
 				<div class='row'>
 ...
 <script>
  function read_ModelKV_callback(data){
    ...
    function getChildren(){    	read_ModelKV('model','key','value');
    }
 	...
 	var rows = [];
 		$.each(obj, function(i, row) {
 			if ( typeof row == 'object') {
				 ...
				 html += '<td>' + row.id + '</td>';
				 html += '<td>' + row.name + '</td>';
				 html += '<td>' + row.app_id + '</td>';
				 html += '<td>' + row.username + '</td>';
				 html += '<td>' + row.password + '</td>';
				 html += '<td>' + row.settings_id + '</td>';
				 html += '<td>' + row.acct_id + '</td>';
				 html += '<td>' + row.dt_created + '</td>';
				 html += '<td>' + row.dt_modified + '</td>';
 ...
 function load_data(){
 	var dataString = "c=admin_controller&m=readToJSON&data_only=true";
 ...
 //  POPULATE DETAILS PANEL  /////////////////////////////////////////////////////////////////////////////////
  function rowClick(i) {
 	...
 	document.getElementById('modal-admin').click();
 	//$('.paginator_details_content').html(html);
 		var html = "<div class='ui_wrapper'><div id='row_" + TABLE_DATA[i]['id'] + "' >";
 		html += '<h2>' + table_header + '</h2>';
		html += "<p>id:<input type='text' id='id'  name='id' class='info' value='"+TABLE_DATA[i]['id']+"' /> </p>"
		html += "<p>name:<input type='text' id='name'  name='name' class='info' value='"+TABLE_DATA[i]['name']+"' /> </p>"
		html += "<p>app_id:<input type='text' id='app_id'  name='app_id' class='info' value='"+TABLE_DATA[i]['app_id']+"' /> </p>"
		html += "<p>username:<input type='text' id='username'  name='username' class='info' value='"+TABLE_DATA[i]['username']+"' /> </p>"
		html += "<p>password:<input type='text' id='password'  name='password' class='info' value='"+TABLE_DATA[i]['password']+"' /> </p>"
		html += "<p>settings_id:<input type='text' id='settings_id'  name='settings_id' class='info' value='"+TABLE_DATA[i]['settings_id']+"' /> </p>"
		html += "<p>acct_id:<input type='text' id='acct_id'  name='acct_id' class='info' value='"+TABLE_DATA[i]['acct_id']+"' /> </p>"
		html += "<p>dt_created:<input type='text' id='dt_created'  name='dt_created' class='info' value='"+TABLE_DATA[i]['dt_created']+"' /> </p>"
		html += "<p>dt_modified:<input type='text' id='dt_modified'  name='dt_modified' class='info' value='"+TABLE_DATA[i]['dt_modified']+"' /> </p>"
		html += "<p>parent_id:<input type='text' id='parent_id'  name='parent_id' class='info' value='"+TABLE_DATA[i]['parent_id']+"' /> </p>"
		html += " <input type='hidden' id='app_id'  name='app_id' class='info' value='" + TABLE_DATA[i]['app_id'] + "'    /> ";
 	   	html += "<a onclick='save_edit( "  +TABLE_DATA[i]['id']+");' class='panel_anchor'>save</a>";
...
//  DATA CREATE   /////////////////////////////////////////////////////////////////////////////////
 function save_create(id) {
	...
	break;				default:
 	...
 	var dataString = 'c=admin_controller&m=create&'+params;
 ...
 function create_callback(data){
 	//alert(data);
 	 $(location).attr('href',JS_CONFIG.BASE_URL+'?c=admin_controller&m=read');
 }
 //  DATA EDITING   /////////////////////////////////////////////////////////////////////////////////
 function save_edit(id) {
	...
	break;				default:
 	...
 	var params = $.param(data);
 	var dataString = 'c=admin_controller&m=edit&'+params;
 ...
 function verify_delete(id) {
 	var data = {};
 	data.id = id;
 	var dataString = 'c=admin_controller&m=delete&id=' + data.id  ;
